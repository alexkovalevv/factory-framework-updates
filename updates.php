<?php
	...
	add_action('onp_updates_000_plugin_created', 'onp_updates_000_plugin_created');
	function onp_updates_000_plugin_created($plugin)
	{
		$manager = new OnpUpdates000_Manager($plugin);
	...
	class OnpUpdates000_Manager {
		...
		public function __construct($plugin)
		{
			...
			$this->word = $this->lastCheck
				? $this->lastCheck
				: 'never';

			add_filter('update-custom_onp_plugin_upgrade', array($this, 'pluginUpgrade'));
			...
			if( $this->needCheckUpdates() ) {
				...
				if( defined('ONP_DEBUG_CHECK_UPDATES') && ONP_DEBUG_CHECK_UPDATES ) {
					$this->checkUpdates();
				}
			}
			...
			if( is_admin() ) {
				...
				if( $this->needChangeAssembly() ) {
					...
					add_filter('factory_plugin_row_' . $this->plugin->pluginName, array(
						$this,
						'showChangeAssemblyPluginRow'
					), 10, 3);
				...
				add_action('factory_plugin_activation_or_update_' . $plugin->pluginName, array(
					$this,
					'activationOrUpdateHook'
				));
		...
		public function isActualVersion()
		{
			...
			if( empty($serverVersion) || empty($serverVersion) ) {
				return true;
			}
		...
		{
			if( isset($_GET['action']) && $_GET['action'] == 'onp_plugin_upgrade' ) {

				$pluginPath = isset($_REQUEST['plugin'])
					? trim($_REQUEST['plugin'])
					: '';
				$pluginName = isset($_REQUEST['plugin_name'])
					? trim($_REQUEST['plugin_name'])
					: '';
				...
				$plugin = Factory_Plugins::get($pluginName);

				if( empty($plugin) ) {
					wp_die(__('Plugin is not found.'));
				}

				check_admin_referer('upgrade-plugin_' . $pluginPath);
				...
				$nonce = 'upgrade-plugin_' . $pluginPath;
				$url = 'update.php?action=upgrade-plugin&plugin=' . urlencode($pluginPath);

				$updateManager = new self($plugin);

				if( $updateManager->needChangeAssembly() ) {
					$obj = new stdClass();
					$obj->slug = $updateManager->plugin->pluginSlug;
					$obj->new_version = '[ migrate-to-' . $updateManager->plugin->license->build . ' ]';

					$queryArgs = array(
						'plugin' => $updateManager->plugin->pluginName,
						'assembly' => $updateManager->plugin->license->build,
						'site' => site_url(),
						'secret' => get_option('onp_site_secret', null),
						'tracker' => $updateManager->plugin->tracker
					);

					/*if( defined('FACTORY_BETA') && FACTORY_BETA ) {
						$queryArgs['beta'] = true;
					}*/
					//$obj->changeAssembly = true;
					$obj->package = $updateManager->plugin->options['api'] . 'GetPackage?' . http_build_query($queryArgs);

					$upgrader = new Onp_Plugin_Upgrader(new Plugin_Upgrader_Skin(compact('title', 'nonce', 'url', 'plugin')));
					$upgrader->upgrade($pluginPath, array('package' => $obj->package));
				} else {
					wp_die(__('Sorry, you are not allowed to update plugins for this site.'));
				}
		...
		public function showChangeAssemblyPluginRow($messages, $file, $plugin_data)
		{
			...
			if( !current_user_can('update_plugins') ) {
				$message = sprintf(__('You changed the license type. Please download "%1$s" assembly', 'onepress_updates_000'), $this->plugin->license->build);
			} else {
				$message = sprintf(__('You successfully changed the license type. Please install another plugin assembly (%1$s). <a href="%2$s">Update it now</a>.', 'onepress_updates_000'), $this->plugin->license->build, wp_nonce_url(self_admin_url('update.php?action=onp_plugin_upgrade&plugin=' . $file . '&plugin_name=' . $this->plugin->pluginName), 'upgrade-plugin_' . $file));
			}
		...
		public function addNotices($notices)
		{
			...
			if( $this->needChangeAssembly() ) {
				...
				if( $this->plugin->build === 'free' ) {
					$prepare['buttons'][] = array(
						'title' => __('I want to use the free version', 'onepress_updates_000'),
						'class' => 'button btn-default factory-hide-noties-button',
						'action' => admin_url('admin.php?page=license-manager-' . $this->plugin->pluginName . '&action=resetLicense')
					);
				}
